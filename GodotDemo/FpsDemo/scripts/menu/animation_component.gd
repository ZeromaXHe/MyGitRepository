...
@export var enter_scale := Vector2.ONE
@export var enter_rotation: float
@export var enter_size: Vector2
@export var enter_modulate := Color.WHITE
...
func connect_signals() -> void:
	target.mouse_entered.connect(add_tween.bind(hover_values, parallel_animations,
		hover_time, hover_delay, hover_transition, hover_easing, false))
	target.mouse_exited.connect(add_tween.bind(default_values, parallel_animations,
		hover_time, hover_delay, hover_transition, hover_easing, false))
	if wait_for:
		wait_for.entered.connect(add_tween.bind(default_values, parallel_animations,
			enter_time, enter_delay, enter_transition, enter_easing, true))
...
func on_enter() -> void:
	...
	if wait_for:
		return
	add_tween(default_values, parallel_animations, enter_time, enter_delay,
		enter_transition, enter_easing, true)
...
func add_tween(values: Dictionary, parallel: bool, seconds: float, delay: float,
	...
	#tween.tween_interval(delay) # 发现这个在 E05 就不行了…… 捂脸
	tween.pause()
