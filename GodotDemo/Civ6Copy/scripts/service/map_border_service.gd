...
static func is_tile_near_border(tile_coord: Vector2i, border_type: MapBorderTable.Enum) -> bool:
	...
	for border in borders:
		if MapService.get_map_border_do_by_coord(border).tile_type == border_type:
			return true
	return false


static func is_river_placeable(border_coord: Vector2i) -> bool:
	if MapShower.singleton.is_in_map_border(border_coord) <= 0 or MapBorderUtils.get_border_type(border_coord) == MapBorderTable.Type.CENTER:
		return false
	...
	for coord in neighbor_tile_coords:
		if not MapService.is_in_map_tile(coord):
			continue
		var terrain_type: TerrainTable.Enum = MapTileService.get_map_tile_do_by_coord(coord).terrain
	...
	for coord in connect_border_coords:
		var border_tile_type: MapBorderTable.Enum = MapService.get_map_border_do_by_coord(coord).tile_type
