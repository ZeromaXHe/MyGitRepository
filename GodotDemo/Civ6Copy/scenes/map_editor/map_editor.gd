		...
		if change.tile_change:
			match change.tile_change_type:
				TileChangeType.TERRAIN:
					...
					MapShower.singleton_minimap.paint_tile(change.coord, change.after)
			...
			# 恢复地图地块信息
			MapTileService.change_map_tile_info(change.coord, change.after)
		else:
			# 恢复 BorderTileMap 到操作后的状态
			map_shower.paint_border(change.coord, change.after_border.tile_type)
			MapShower.singleton_minimap.paint_border(change.coord, change.after_border.tile_type)
			# 恢复地图地块信息
		...
		if change.tile_change:
			match change.tile_change_type:
				TileChangeType.TERRAIN:
					...
					MapShower.singleton_minimap.paint_tile(change.coord, change.before)
				TileChangeType.LANDSCAPE:
					map_shower.paint_tile(change.coord, change.before)
				TileChangeType.VILLAGE:
			...
			# 还原地图地块信息
			MapTileService.change_map_tile_info(change.coord, change.before)
		else:
			# 恢复 BorderTileMap 到操作前的状态
			map_shower.paint_border(change.coord, change.before_border.tile_type)
			MapShower.singleton_minimap.paint_border(change.coord, change.before_border.tile_type)
			# 恢复地图地块信息
...
func paint_new_chosen_area(renew: bool = false) -> void:
	...
	match gui.place_mode:
		MapEditorGUI.PlaceMode.RIVER:
			map_shower.paint_chosen_border_area(border_coord, MapBorderService.is_river_placeable)
		MapEditorGUI.PlaceMode.CLIFF:
			map_shower.paint_chosen_border_area(border_coord, MapBorderService.is_cliff_placeable)
		MapEditorGUI.PlaceMode.TERRAIN:
			var dist: int = gui.get_painter_size_dist()
			var new_inside: Array[Vector2i] = MapService.get_surrounding_cells(map_coord, dist, true)
			for coord in new_inside:
				# 新增新图块
				map_shower.paint_tile_chosen_placeable(coord)
		MapEditorGUI.PlaceMode.LANDSCAPE:
			var placeable: Callable = func(x) -> bool: return LandscapeService.is_landscape_placeable(x, gui.landscape_type)
			map_shower.paint_chosen_tile_area(map_coord, placeable)
		MapEditorGUI.PlaceMode.VILLAGE:
			map_shower.paint_chosen_tile_area(map_coord, VillageService.is_village_placeable)
		MapEditorGUI.PlaceMode.RESOURCE:
			var placeable: Callable = func(x) -> bool: return ResourceService.is_resource_placeable(x, gui.resource_type)
			map_shower.paint_chosen_tile_area(map_coord, placeable)
		MapEditorGUI.PlaceMode.CONTINENT:
			var dist: int = gui.get_painter_size_dist()
			var new_inside: Array[Vector2i] = MapService.get_surrounding_cells(map_coord, dist, true)
			for coord in new_inside:
				map_shower.paint_chosen_tile_area(coord, ContinentService.is_continent_placeable)
...
func depaint_map() -> void:
	...
	match gui.place_mode:
		MapEditorGUI.PlaceMode.CLIFF:
			var border_coord: Vector2i = map_shower.get_border_coord()
			if MapService.get_map_border_do_by_coord(border_coord).tile_type != MapBorderTable.Enum.CLIFF:
				return
		...
		MapEditorGUI.PlaceMode.RIVER:
			var border_coord: Vector2i = map_shower.get_border_coord()
			if MapService.get_map_border_do_by_coord(border_coord).tile_type != MapBorderTable.Enum.RIVER:
				return
			paint_border(border_coord, step, MapBorderTable.Enum.EMPTY)
			# 强制重绘选择区域
			paint_new_chosen_area(true)
		MapEditorGUI.PlaceMode.LANDSCAPE:
			...
			if MapTileService.get_map_tile_do_by_coord(tile_coord).landscape == LandscapeTable.Enum.EMPTY:
				return
		...
		MapEditorGUI.PlaceMode.VILLAGE:
			var tile_coord: Vector2i = map_shower.get_mouse_map_coord()
			if not MapTileService.get_map_tile_do_by_coord(tile_coord).village:
				return
		...
		MapEditorGUI.PlaceMode.RESOURCE:
			var tile_coord: Vector2i = map_shower.get_mouse_map_coord()
			if MapTileService.get_map_tile_do_by_coord(tile_coord).resource == ResourceTable.Enum.EMPTY:
...
func paint_map() -> void:
	...
	if gui.get_rt_tab_status() == MapEditorGUI.TabStatus.GRID:
		...
		map_shower.paint_tile_chosen_placeable(coord)
		gui.update_grid_info(coord, MapTileService.get_map_tile_do_by_coord(coord))
		return
	var step: PaintStep = PaintStep.new()
	match gui.place_mode:
		MapEditorGUI.PlaceMode.TERRAIN:
			var map_coord: Vector2i = map_shower.get_mouse_map_coord()
			var dist: int = gui.get_painter_size_dist()
			var inside: Array[Vector2i] = MapService.get_surrounding_cells(map_coord, dist, true)
			for coord in inside:
				# 超出地图范围的不处理
				if not MapService.is_in_map_tile(coord):
					continue
				if MapTileService.get_map_tile_do_by_coord(coord).terrain == gui.terrain_type:
					continue
			...
			if gui.terrain_type != TerrainTable.Enum.SHORE and gui.terrain_type != TerrainTable.Enum.OCEAN:
				var out_ring: Array[Vector2i] = MapService.get_surrounding_cells(map_coord, dist + 1, false)
				for coord in out_ring:
					# 超出地图范围的不处理
					if not MapService.is_in_map_tile(coord):
						continue
					# 仅深海需要改为浅海
					if MapTileService.get_map_tile_do_by_coord(coord).terrain != TerrainTable.Enum.OCEAN:
						continue
			...
			if TerrainService.is_hill_land_terrain(gui.terrain_type):
				var borders: Array[Vector2i] = MapBorderService.get_surrounding_borders(map_coord, dist)
				for border in borders:
					if MapBorderService.is_cliff_placeable(border):
						paint_border(border, step, MapBorderTable.Enum.CLIFF)
			# 强制重绘选择区域
			paint_new_chosen_area(true)
		MapEditorGUI.PlaceMode.LANDSCAPE:
			var coord: Vector2i = map_shower.get_mouse_map_coord()
			if not LandscapeService.is_landscape_placeable(coord, gui.landscape_type):
				return
			if MapTileService.get_map_tile_do_by_coord(coord).landscape == gui.landscape_type:
				return
		...
		MapEditorGUI.PlaceMode.VILLAGE:
			var coord: Vector2i = map_shower.get_mouse_map_coord()
			if not VillageService.is_village_placeable(coord):
				return
			if MapTileService.get_map_tile_do_by_coord(coord).village:
				return
		...
		MapEditorGUI.PlaceMode.RESOURCE:
			var coord: Vector2i = map_shower.get_mouse_map_coord()
			if not ResourceService.is_resource_placeable(coord, gui.resource_type):
				return
			if MapTileService.get_map_tile_do_by_coord(coord).resource == gui.resource_type:
				return
		...
		MapEditorGUI.PlaceMode.CONTINENT:
			...
			var inside: Array[Vector2i] = MapService.get_surrounding_cells(map_coord, dist, true)
			for coord in inside:
				if not ContinentService.is_continent_placeable(coord):
					continue
				if MapTileService.get_map_tile_do_by_coord(coord).continent == gui.continent_type:
					continue
		...
		MapEditorGUI.PlaceMode.CLIFF:
			var border_coord: Vector2i = map_shower.get_border_coord()
			if not MapBorderService.is_cliff_placeable(border_coord):
				return
			if MapService.get_map_border_do_by_coord(border_coord).tile_type == MapBorderTable.Enum.CLIFF:
				return
			# 绘制边界悬崖
		...
		MapEditorGUI.PlaceMode.RIVER:
			var border_coord: Vector2i = map_shower.get_border_coord()
			if not MapBorderService.is_river_placeable(border_coord):
				return
			if MapService.get_map_border_do_by_coord(border_coord).tile_type == MapBorderTable.Enum.RIVER:
				return
			# 绘制边界河流
...
func paint_terrain(coord: Vector2i, step: PaintStep, terrain_type: TerrainTable.Enum):
	...
	if change.before.landscape != LandscapeTable.Enum.EMPTY \
			and not LandscapeService.is_landscape_placeable_terrain(change.before.landscape, terrain_type):
		change.after.landscape = LandscapeTable.Enum.EMPTY
	if change.before.village and not VillageService.is_village_placeable_terrain(terrain_type):
		change.after.village = false
	
	if change.before.continent != ContinentTable.Enum.EMPTY \
			and not ContinentService.is_continent_placeable_terrain(terrain_type):
		change.after.continent = ContinentTable.Enum.EMPTY
	elif ContinentService.is_continent_placeable_terrain(terrain_type) and change.before.continent == ContinentTable.Enum.EMPTY:
		# 从海变陆时需要给个默认的大洲
		change.after.continent = gui.continent_type
	
	if change.before.resource != ResourceTable.Enum.EMPTY \
			and not ResourceService.is_resource_placeable_terrain_and_landscape(change.before.resource, terrain_type, change.after.landscape):
		change.after.resource = ResourceTable.Enum.EMPTY
	
	step.changed_arr.append(change)
	
	# 记录地图地块信息
	MapTileService.change_map_tile_info(coord, change.after)
	# 重新绘制整个 TileMap 地块
	map_shower.paint_tile(coord, change.after)
	MapShower.singleton_minimap.paint_tile(coord, change.after)
...
func paint_landscape(tile_coord: Vector2i, step: PaintStep, type: LandscapeTable.Enum) -> void:
	...
	if change.before.resource == ResourceTable.Enum.EMPTY \
			and not ResourceService.is_resource_placeable_terrain_and_landscape(change.before.resource, change.before.terrain, type):
		change.after.resource = ResourceTable.Enum.EMPTY
		map_shower.paint_resource(tile_coord, ResourceTable.Enum.EMPTY)
	
	step.changed_arr.append(change)
	# 记录地图地块信息
	MapTileService.change_map_tile_info(tile_coord, change.after)
	# 真正绘制地貌
...
func paint_border(border_coord: Vector2i, step: PaintStep, type: MapBorderTable.Enum) -> void:
	...
	MapShower.singleton_minimap.paint_border(border_coord, type)
...
func build_change_of_tile(tile_coord: Vector2i, type: TileChangeType) -> PaintChange:
	...
	change.before = MapTileService.get_map_tile_do_by_coord(tile_coord)
