...
将 Viewport 想象为游戏投影到的屏幕。为了观看比赛，我们需要有一个可以绘制比赛的表面；该曲面就是根视口。

视口也可以添加到场景中，这样就有多个曲面可以绘制。当我们绘制到不是根的视口时，我们称之为渲染目标。我们可以通过访问渲染目标的相应纹理来访问其内容。通过使用 Viewport 作为渲染目标，我们可以同时渲染多个场景，也可以渲染到应用于场景中对象的纹理，例如动态 skybox。
...
视口还负责向其子节点传递经过适当调整和缩放的输入事件。默认情况下，子视口不会自动接收输入，除非它们从直接的子视口容器父节点接收输入。在这种情况下，可以使用“禁用输入”属性禁用输入。

有关 Godot 如何处理输入的更多信息，请阅读输入事件教程。

#### 监听器

Godot 支持 3D 声音（在 2D 和 3D 节点中）；有关这方面的更多信息，请参阅音频流教程。要使这种类型的声音听得见，需要启用 Viewport 作为侦听器（对于 2D 或 3D）。如果您使用自定义视口来显示 World3D 或 World2D，请不要忘记启用此功能！

#### 相机（2D 和 3D）

使用Camera3D/Camera2D时，摄影机将始终显示在最近的父视口上（朝向根视口）。例如，在以下层次结构中：

CameraA 将显示在根视口上，它将绘制 MeshA。CameraB 将与 MeshB 一起由 Viewport 节点捕获。即使 MeshB 在场景层次中，它仍然不会被绘制到根视口。类似地，MeshA 在 Viewport 节点中将不可见，因为 Viewport 节点仅捕获层次中其下方的节点。

每个 Viewport 只能有一个活动摄影机，因此如果有多个，请确保所需的摄影机具有“当前”特性集，或者通过调用使其成为当前摄影机：

```python
camera.make_current()
```

默认情况下，摄影机将渲染其世界中的所有对象。在 3D 中，摄影机可以将其 cull_mask 特性与 VisualInstance3D 的层特性结合使用，以限制渲染哪些对象。

#### 缩放和拉伸

视口具有“大小”属性，该属性表示视口的大小（以像素为单位）。对于子视口容器的子视口，这些值将被覆盖，但对于所有其他视口，这将设置其分辨率。

也可以缩放二维内容，并使“视口”分辨率与指定的大小不同，方法是调用：
...
根视口将其用于项目设置中的拉伸选项。有关缩放和拉伸的更多信息，请访问“多分辨率教程”

#### 世界

对于 3D，视口将包含 World3D。这基本上就是将物理和渲染联系在一起的宇宙。基于 Node3D 的节点将使用最近 Viewport 的 World3D 进行注册。默认情况下，新创建的视口不包含 World3D，而是使用与其父视口相同的视口（根视口始终包含 World3D（默认情况下渲染对象的对象）。可以使用 “world” 属性在 Viewport 中设置 World3D，这将使该 Viewport 的所有子节点与父 Viewport 的 World3D 交互。这在场景中尤其有用，例如，你可能想在游戏中显示一个单独的 3D 角色（比如《星际争霸》）。

作为要创建显示单个对象的视口而不想创建 World3D 的情况下的辅助对象，Viewport 可以选择使用自己的 World3D。当您希望在二维世界二维中实例化三维角色或对象时，这很有用。

对于 2D，每个视口始终包含自己的 World2D。在大多数情况下，这就足够了，但在可能需要共享它们的情况下，可以通过手动设置 Viewport 的 World2D 来实现。

有关如何工作的示例，请分别参见演示项目 3D in 2D 和 2D in 3D。

#### 捕获

可以查询 Viewport 内容的捕获。对于根 Viewport，这实际上是一个屏幕捕获。这是通过以下代码完成的：
...
但是，如果在 `_ready()` 中或在 Viewport 初始化的第一帧中使用此选项，则会得到一个空纹理，因为没有什么可以作为纹理获得。您可以使用（例如）：
...
如果 Viewport 是 SubViewportContainer 的子对象，则它将变为活动状态并显示其内部的任何内容。布局如下所示：

如果在 SubViewportContainer 中将“拉伸”设置为 `true`，则 Viewport 将完全覆盖其父 SubViewportContainer 的区域。注意：子视口容器的大小不能小于视口的大小。

#### 渲染

由于 Viewport 是另一个渲染曲面的入口，因此它公开了一些与项目设置不同的渲染特性。第一种是 MSAA；您可以选择为每个 Viewport 使用不同级别的 MSAA；默认行为为 DISABLED。您也可以将“视口”设置为使用 HDR，当您希望在 0.0 - 1.0 范围之外的纹理中存储值时，HDR 非常有用。

如果知道视口的使用方式，可以将其“用途”设置为三维或二维。Godot 将根据您的选择限制视口的绘制方式；默认为 3D。与 3D 模式相比，2D 使用模式稍微快一些，并且使用更少的内存。如果视口不以三维方式渲染任何内容，最好将“视口的用途”属性设置为二维。

> **注意：**
>
> 如果需要在视口中渲染三维阴影，请确保将视口的“*阴影图集大小*”属性设置为大于 0 的值。否则，将不会渲染阴影。作为参考，项目设置默认情况下将其定义为 4096。

Godot 还提供了一种使用“调试绘制”自定义 Viewports 内部绘制所有内容的方式。“调试绘图”允许您指定四个选项中的一个，用于视口如何显示其中绘制的内容。默认情况下，“调试绘制”处于禁用状态。

其他三个选项是“Unshaded”、“Overdraw” 和“线框”。Unshaded 在不使用照明信息的情况下绘制场景，因此所有对象都显示为其反照率的颜色。

Overdraw 使用添加混合绘制半透明网格，以便您可以看到网格如何重叠。

最后，“线框”仅使用网格中三角形的边绘制场景。

> **注意：**
>
> 线框模式的效果仅在编辑器中可见，在项目运行时不可见。

#### 渲染目标

当渲染到 Viewport 时，里面的任何东西在场景编辑器中都不可见。若要显示内容，必须在某个位置绘制 Viewport 的 ViewportTexture。这可以通过以下代码请求（例如）：
...
或者可以在编辑器中通过选择 “New ViewportTexture” 来指定

然后选择要使用的视口。

每帧，Viewport 的纹理都会使用默认的透明颜色（如果“透明 Bg”设置为 `true`，则为透明颜色）清除。这可以通过将 `Clear Mode`（“清除模式”）设置为“从不”或“下一帧”来更改。顾名思义，Never 表示纹理永远不会被清除，而下一帧将清除下一帧上的纹理，然后将其自身设置为 Never。

默认情况下，当在帧中绘制 Viewport 的 ViewportTexture 时，会重新渲染 Viewport。如果可见，将对其进行渲染；否则，它不会。此行为可以更改为手动渲染（一次），也可以始终渲染，无论是否可见。这种灵活性允许用户渲染一次图像，然后使用纹理，而不会产生渲染每帧的成本。

请务必检查 Viewport 演示！演示档案中的 Viewport 文件夹可供下载，或 https://github.com/godotengine/godot-demo-projects/tree/master/viewport

### 多分辨率

#### 多分辨率问题

开发人员经常难以理解如何在游戏中最好地支持多分辨率。对于台式机和主机游戏来说，这或多或少是简单的，因为大多数屏幕纵横比是 16:9，分辨率是标准的（720p、1080p、1440p、4K…）。

对于手机游戏来说，一开始很容易。多年来，iPhone 和 iPad 使用相同的分辨率。当 Retina 被实现时，他们只是将像素密度增加了一倍；大多数开发商不得不在默认和两倍分辨率的情况下提供资产。

如今，情况已不再如此，因为有很多不同的屏幕尺寸、密度和纵横比。非传统尺寸也越来越受欢迎，例如超宽显示器。

对于 3D 游戏，不需要支持多分辨率（从美学角度来看）。3D 几何体将仅根据视野填充屏幕，而不考虑纵横比。在这种情况下，人们可能想要支持它的主要原因是出于性能原因（以较低的分辨率运行以增加每秒帧数）。

对于 2D 和游戏 UI 来说，这是另一回事，因为艺术需要在 Photoshop、GIMP 或 Krita 等软件中使用特定的像素大小来创作。

由于布局、纵横比、分辨率和像素密度可能变化很大，因此不再可能为每个特定屏幕设计 UI。必须使用另一种方法。

#### 一刀切

最常见的方法是使用单一的基本分辨率，然后将其与其他所有分辨率相匹配。这个决议是大多数玩家玩游戏的方式（考虑到他们的硬件）。对于移动设备，谷歌有有用的在线统计数据，而对于台式机，Steam 也有。

例如，Steam 显示，最常见的*主显示器分辨率*为 1920×1080，因此一个明智的方法是为该分辨率开发一款游戏，然后处理不同尺寸和纵横比的缩放。

Godot 提供了几个有用的工具，可以轻松地做到这一点。

> **参考：**
>
> 您可以使用 multiple resolutions and Aspect Ratios 演示项目了解 Godot 对多分辨率的支持是如何发挥作用的。

#### 底座尺寸

可以在“项目设置”下的**“显示 → 窗口”**中指定窗口的基本尺寸。

然而，它的作用并不完全显而易见；发动机将*不会*尝试将监视器切换到此分辨率。相反，可以将此设置视为“设计大小”，即您在编辑器中使用的区域的大小。此设置直接对应于 2D 编辑器中蓝色矩形的大小。

通常需要支持屏幕和窗口大小与此基本大小不同的设备。Godot 提供了许多方法来控制视口的大小和拉伸到不同屏幕大小的方式。

要在运行时从脚本配置拉伸基本大小，请使用 `get_tree().root.content_scale_size` 属性（请参阅 Window.content_scale_size）。更改此值可以间接更改 2D 元素的大小。但是，为了提供用户可访问的缩放选项，建议使用“拉伸比例”，因为它更容易调整。

> **注意：**
>
> Godot 采用现代方法处理多个分辨率。引擎永远不会自行更改监视器的分辨率。虽然改变显示器的分辨率是最有效的方法，但这也是最不可靠的方法，因为如果游戏崩溃，它可能会使显示器停留在低分辨率。这在 macOS 或 Linux 上尤其常见，因为它们不能像 Windows 那样处理分辨率更改。
>
> 更改显示器的分辨率还可以消除游戏开发商对过滤和纵横比拉伸的任何控制，这对于确保像素艺术游戏的正确显示非常重要。
>
> 除此之外，更改显示器的分辨率会使进出游戏的 alt-tab 速度慢得多，因为显示器每次都必须更改分辨率。

#### 调整大小

有几种类型的设备和几种类型的屏幕，这些屏幕又具有不同的像素密度和分辨率。处理所有这些都可能是一项艰巨的工作，所以 Godot 试图让开发人员的生活更轻松一点。Viewport 节点有几个函数来处理大小调整，并且场景树的根节点始终是一个视口（加载的场景被实例化为其子节点，并且始终可以通过调用 `get_tree().root` 或 `get_node("/root")` 来访问它）。

无论如何，虽然更改根 Viewport 参数可能是处理该问题最灵活的方法，但这可能需要大量的工作、代码和猜测，因此 Godot 在项目设置中提供了一组参数来处理多个分辨率。

#### 拉伸设置

拉伸设置位于项目设置中，并提供了几个选项：

##### 拉伸模式

“**拉伸模式**”设置定义如何拉伸基本大小以适应窗口或屏幕的分辨率。

下面的动画使用 16×9 像素的“基本尺寸”来演示不同拉伸模式的效果。一个大小为 16×9 像素的小精灵覆盖了整个视口，并在其顶部添加了一个对角线 Line2D：

- **拉伸模式 = 禁用**（默认）：不进行拉伸。场景中的一个单元对应于屏幕上的一个像素。在该模式下，“**拉伸纵横比**”设置无效。
- **拉伸模式 = 画布项目**：在此模式下，在项目设置中以宽度和高度指定的基本大小将被拉伸以覆盖整个屏幕（考虑**拉伸纵横比**设置）。这意味着所有内容都直接以目标分辨率进行渲染。3D 不受影响，而在 2D 中，精灵像素和屏幕像素之间不再存在 1:1 的对应关系，这可能会导致缩放伪影。
- **拉伸模式 = 视口**：视口缩放意味着根视口的大小精确设置为“项目设置”的“**显示**”部分中指定的基本大小。场景首先渲染到此视口。最后，缩放此视口以适应屏幕（考虑到“**拉伸纵横比**”设置）。

要在运行时从脚本配置拉伸模式，请使用 `get_tree().root.content_scale_mode` 属性（请参阅 Window.content_scale_mode 和 ContentScaleMode 枚举）。

##### 拉伸纵横比

第二个设置是拉伸方面。请注意，只有将“**拉伸模式**”设置为“**禁用**”以外的值时，此选项才会生效。

在下面的动画中，您会注意到灰色和黑色区域。黑色区域是由引擎添加的，无法绘制。灰色区域是场景的一部分，可以绘制到。灰色区域对应于您在 2D 编辑器中看到的蓝色框架之外的区域。

- **拉伸纵横比 = 忽略**：拉伸屏幕时忽略纵横比。这意味着原始分辨率将被拉伸以完全填充屏幕，即使屏幕更宽或更窄。这可能会导致拉伸不均匀：东西看起来比设计的更宽或更高。

- **拉伸纵横比 = 保持**：拉伸屏幕时保持纵横比。这意味着无论屏幕分辨率如何，视口都将保持其原始大小，并且黑条将添加到屏幕的顶部/底部（“letterboxing”）或侧面（“pillarboxing”）。

  如果你提前知道目标设备的纵横比，或者你不想处理不同的纵横比的话，这是一个很好的选择。

- **拉伸纵横比 = 保持宽度**：拉伸屏幕时保持纵横比。如果屏幕比基本尺寸宽，则会在左右两侧添加黑条（柱框）。但是，如果屏幕高于基本分辨率，视口将沿垂直方向增长（底部将看到更多内容）。您也可以将其视为“垂直展开”。

  这通常是创建可缩放的GUI或HUD的最佳选项，因此一些控件可以锚定在底部（大小和锚定）。

- **拉伸纵横比 = 保持高度**：拉伸屏幕时保持纵横比。如果屏幕比基本尺寸高，则会在顶部和底部添加黑条（信箱）。但是，如果屏幕比基本分辨率宽，视口将在水平方向上增长（右侧将可见更多内容）。您也可以将其视为“横向扩展”。

  这通常是水平滚动的 2D 游戏（如跑酷游戏或平台游戏）的最佳选择。

- **拉伸纵横比 = 展开**：拉伸屏幕时保持纵横比，但不保持基本宽度和高度。根据屏幕纵横比的不同，视口在水平方向上（如果屏幕比基本尺寸宽）或在垂直方向上（屏幕比原始尺寸高）会更大。

> **提示：**
>
> 要使用类似的自动确定的比例因子同时支持纵向和横向模式，请将项目的基本分辨率设置为正方形（1:1 纵横比），而不是矩形。例如，如果您希望将 1280×720 设计为基本分辨率，但希望同时支持纵向和横向模式，请在“项目设置”中使用 720×720 作为项目的基本窗口大小。
> 要允许用户在运行时选择他们喜欢的屏幕方向，请记住将**“显示”>“窗口”>“手持设备”>“方向”**设置为 `sensor`。

要在运行时从脚本配置拉伸特性，请使用 `get_tree().root.content_scale_aspect` 属性（请参见 Window.content_scale_aspect 和 ContentScaleAspect 枚举）。

##### 拉伸比例

“**缩放**”设置允许您在上面的“**拉伸**”选项已经提供的基础上添加额外的缩放因子。默认值为 `1.0` 意味着不会发生其他缩放。

例如，如果将“**缩放**”设定为 `2.0`，并将“**拉伸模式**”保持为“**禁用**”，则场景中的每个单位将对应于屏幕上的 2×2 像素。这是为非游戏应用程序提供缩放选项的好方法。

如果“**拉伸模式**”设置为 **canvas_items**，则二维元素将相对于基本窗口大小进行缩放，然后乘以“**缩放**”设置。这可以向玩家展示，让他们根据自己的喜好调整自动确定的比例，以获得更好的可访问性。

如果“**拉伸模式**”设置为**视口**，则视口的分辨率将除以“**缩放**”。这会使像素看起来更大，并降低渲染分辨率（在给定窗口大小的情况下），从而提高性能。

要在运行时从脚本配置拉伸比例，请使用 `get_tree().root.content_scale_factor` 属性（请参见 Window.content_scale_factor）。

#### 常见用例场景

建议使用以下设置来很好地支持多种分辨率和纵横比。

##### 桌面游戏

**非像素艺术：**

- 将基本窗口宽度设置为 `1920`，将窗口高度设置为 `1080`。如果显示器小于 1920×1080，请将“**测试宽度**”和“**测试高度**”设置为较低的值，以便在项目启动时使窗口变小。
- 或者，如果您主要针对高端设备，请将基本窗口宽度设置为 `3840`，将窗口高度设置为 `2160`。这使您能够提供更高分辨率的2D资产，从而以更高的内存使用率和文件大小为代价获得更清晰的视觉效果。请注意，这将使非 mipmap 纹理在低分辨率设备上具有颗粒状，因此请确保按照减少下采样时的混叠中所述的说明进行操作。
- 将拉伸模式设置为 `canvas_items`。
- 将拉伸方面设置为 `expand`。这允许支持多个纵横比，并更好地利用高大的智能手机显示屏（如 18:9 或 19:9 纵横比）。
- 使用“**布局**”菜单配置控制节点的锚点以捕捉到正确的角点。

**像素艺术：**

- 将基本窗口大小设置为要使用的视口大小。大多数像素艺术游戏使用的视口大小在 256×224 到 640×480 之间。更高的视口大小将需要使用更高分辨率的艺术作品，除非您打算在给定的时间显示更多的游戏世界。
- 将拉伸模式设置为 `viewport`。
- 将拉伸纵横比设置为 `keep`，以强制执行单个纵横比（使用黑条）。或者，可以将拉伸纵横比设置为 `expand` 以支持多个纵横比。
- 如果使用 `expand` 拉伸方式，请使用“**布局**”菜单配置控制节点的锚点以捕捉到正确的角点。

> **注意：**
>
> `viewport` 拉伸模式提供低分辨率渲染，然后将其拉伸到最终窗口大小。如果您同意精灵能够在“子像素”位置移动或旋转，或者希望拥有高分辨率的 3D 视口，则应使用 `canvas_items` 拉伸模式而不是 `viewport` 拉伸模式。
>
> Godot 目前在使用 `canvas_items` 或 `viewport` 拉伸模式时无法强制执行整数缩放，这意味着如果最终窗口大小不是基本窗口大小的倍数，像素艺术可能看起来很糟糕。要解决此问题，请使用诸如 Integer Resolution Handler 之类的附加组件。

##### 横向模式下的手机游戏

Godot 默认配置为使用横向模式。这意味着您不需要更改显示方向项目设置。

- 将基本窗口宽度设置为 `1280`，将窗口高度设置为 `720`。
- 或者，如果您主要针对高端设备，请将基本窗口宽度设置为 `1920`，将窗口高度设置为 `1080`。这使您能够提供更高分辨率的 2D 资产，从而以更高的内存使用率和文件大小为代价获得更清晰的视觉效果。许多设备的显示屏分辨率甚至更高（1440p），但考虑到智能手机显示屏的小尺寸，与 1080p 的区别几乎看不出来。请注意，这将使非 mipmap 纹理在低分辨率设备上具有颗粒状，因此请确保按照减少下采样时的混叠中所述的说明进行操作。
- 将拉伸模式设置为 `canvas_items`。
- 将拉伸方面设置为 `expand`。这允许支持多个纵横比，并更好地利用高大的智能手机显示屏（如18:9或19:9纵横比）。
- 使用“**布局**”菜单配置控制节点的锚点以捕捉到正确的角点。

> **提示：**
>
> 为了更好地支持平板电脑和可折叠手机（它们通常具有纵横比接近4:3的显示器），请考虑使用纵横比为 4:3 的基本分辨率，同时遵循此处的其余说明。例如，可以将基本窗口宽度设置为 `1280`，将基本窗口高度设置为 `960`。

##### 纵向模式下的手机游戏

- 将基本窗口宽度设置为 `720`，将窗口高度设置为 `1280`。
- 或者，如果您主要针对高端设备，请将基本窗口宽度设置为 `1080`，将窗口高度设置为 `1920`。这使您能够提供更高分辨率的 2D 资产，从而以更高的内存使用率和文件大小为代价获得更清晰的视觉效果。许多设备的显示屏分辨率甚至更高（1440p），但考虑到智能手机显示屏的小尺寸，与 1080p 的区别几乎看不出来。请注意，这将使非 mipmap 纹理在低分辨率设备上具有颗粒状，因此请确保按照减少下采样时的混叠中所述的说明进行操作。
...
> **提示：**
>
> 为了更好地支持平板电脑和可折叠手机（它们通常具有纵横比接近 4:3 的显示器），请考虑使用纵横比为 3:4 的基本分辨率，同时遵循此处的其余说明。例如，可以将基本窗口宽度设置为 `960`，将基本窗口高度设置为 `1280`。

##### 非游戏应用程序

- 将基本窗口的宽度和高度设置为要设定目标的最小窗口大小。这不是必需的，但这可以确保您在设计UI时考虑到小窗口大小。
- 将拉伸模式保持为其默认值（`disabled`）。
- 将拉伸方面保持为其默认值，`ignore`（由于拉伸模式处于 `disabled`，因此不会使用其值）。
- 您可以通过在脚本的 `_ready()` 函数中设置 `OS.min_window_size` 来定义最小窗口大小。这可以防止用户将应用程序的大小调整到低于特定大小，这可能会破坏 UI 布局。

> **注意：**
>
> Godot 还不支持手动覆盖 2D 比例因子，因此在非游戏应用程序中不可能支持 hiDPI。因此，建议在非游戏应用程序中禁用“**允许 Hidpi**”，以允许操作系统使用其低 DPI 回退。

#### hiDPI 支持

默认情况下，操作系统不认为 Godot 项目具有 DPI 意识。这样做是为了提高低端系统的性能，因为操作系统的 DPI 回退缩放将比应用程序自己缩放更快（即使使用 `viewport` 拉伸模式）。

然而，操作系统提供的 DPI 回退缩放在全屏模式下效果不佳。如果您希望在 hiDPI 显示器上显示清晰的视觉效果，或者项目使用全屏，建议在项目设置中启用**“显示” > “窗口” > “Dpi” > “允许 hiDPI”**。

**允许 Hidpi** 仅在 Windows 和 macOS 上有效。它在所有其他平台上都被忽略。

> **注意：**
>
> Godot 编辑器本身始终标记为 DPI 感知。如果在“项目设置”中启用了“**允许 Hidpi**”，则从编辑器中运行项目将仅支持 DPI。

#### 减少下采样时的锯齿

如果游戏的基本分辨率非常高（例如 3840×2160），则当向下采样到 1280×720 等相当低的分辨率时，可能会出现锯齿。

若要解决此问题，可以在所有 2D 纹理上启用 mipmaps。然而，启用 mipmaps 将增加内存使用量，这在低端移动设备上可能会出现问题。

#### 处理纵横比

一旦考虑到了不同分辨率的缩放，请确保您的*用户界面*也可以根据不同的纵横比进行缩放。这可以使用锚和/或容器来完成。

#### 视野缩放

三维摄影机节点的“**保持纵横比**”特性默认为“**保持高度**”缩放模式（也称为 *Hor+*）。这通常是横向模式下桌面游戏和手机游戏的最佳价值，因为宽屏幕显示器将自动使用更宽的视野。

然而，如果你的 3D 游戏是在人像模式下玩的，那么使用“**保持宽度**”（也称为 *Vert-*）可能更有意义。这样，纵横比高于 16:9（例如 19:9）的智能手机将使用更高的视野，这在这里更合乎逻辑。

#### 使用视口以不同方式缩放二维和三维元素

使用多个“视口”节点，可以为各种图元设置不同的比例。例如，可以使用此选项以低分辨率渲染三维世界，同时将二维元素保持为本机分辨率。这可以显著提高性能，同时保持 HUD 和其他 2D 元素的清晰。

这是通过仅对 2D 元素使用根 Viewport 节点来完成的，然后创建一个 Viewport 节点以显示 3D 世界，并使用 SubViewportContainer 或 TextureRect 节点来显示它。最终项目中将有效地存在两个视口。在 SubViewportContainer 上使用 TextureRect 的一个好处是它允许启用线性过滤。这使得缩放后的三维视口在许多情况下看起来更好。
...
*抖动*和*卡顿*是屏幕上物体可见运动的两种不同变化，即使在全速运行时也可能影响游戏。这些效果在世界以恒定速度向固定方向移动的游戏中最为明显，比如跑酷游戏或平台游戏。

*输入滞后*与抖动和卡顿无关，但有时也会与之一起讨论。输入滞后是指在使用鼠标、键盘、控制器或触摸屏执行操作时，屏幕上可见的延迟。它可以与游戏代码、引擎代码或外部因素（如硬件）有关。输入滞后在使用鼠标瞄准的游戏中最为明显，例如第一人称游戏。输入滞后不能完全消除，但可以通过几种方式来减少。

#### 区分抖动和卡顿

以正常帧速率运行而不显示任何效果的游戏将显得流畅：

出现抖动的游戏会以一种非常微妙的方式不断抖动：

最后，出现卡顿的游戏会看起来很流畅，但似乎每隔几秒钟就会停止或回滚一帧：

#### 抖动

抖动的原因有很多，最典型的是当游戏*物理频率*（通常为 60 Hz）以与显示器刷新率不同的分辨率运行时。检查您的显示器刷新率是否与 60 Hz 不同。

这通常不是问题，因为大多数显示器都是60Hz，并且从 Godot 3.1 开始，引入了一个帧计时器，试图尽可能与刷新同步。

有时只有某些对象出现抖动（角色或背景）。当它们在不同的时间源中处理时，就会发生这种情况（一个在物理步骤中处理，另一个在空闲步骤中处理）。Godot 3.1 对此做了一些改进，从允许在常规 `_process()` 循环中为运动学实体设置动画，到在帧计时器中进行进一步修复。

> **注意：**
>
> 可以使用物理插值来减轻与物理相关的抖动。请参阅 lawnjelly 的平滑插件，了解可以放入任何项目中以启用物理插值的插件。

#### 卡顿

卡顿的发生可能有两个不同的原因。第一个，也是最明显的一个，是游戏无法保持完整的帧速率性能。解决这个问题是特定于游戏的，需要优化。

第二个更复杂，因为它通常与引擎或游戏无关，而是与底层操作系统有关。以下是关于不同操作系统上的卡顿的一些信息。

在支持禁用 V-Sync 的平台上，可以通过在项目设置中禁用 V-Sync 来减少卡顿。然而，这会导致撕裂，尤其是在刷新率较低的显示器上。如果您的显示器支持它，请考虑在启用 V-Sync 的同时启用可变刷新率（G-Sync / FreeSync）。这避免了在不引起撕裂的情况下减轻某些形式的卡顿。

强制您的显卡使用最大性能配置文件也有助于减少卡顿，代价是增加 GPU 功耗。

##### Windows

众所周知，Windows 会导致有窗口的游戏出现卡顿。这主要取决于安装的硬件、驱动程序版本和并行运行的进程（例如，打开许多浏览器选项卡可能会导致运行中的游戏出现停顿）。为了避免这种情况，从 3.1 开始，Godot 将游戏优先级提高到“高于正常”。这有很大帮助，但可能无法完全消除卡顿。

完全消除这种情况需要给你的游戏充分的特权，让它变得“时间关键”，这是不建议的。有些游戏可能会这样做，但建议学会面对这个问题，因为这在 Windows 游戏中很常见，大多数用户不会在窗口中玩游戏（在窗口中播放的游戏，例如益智游戏，通常不会出现这个问题）。

对于全屏，Windows 对游戏给予了特殊的优先级，因此卡顿不再可见，而且非常罕见。这就是大多数游戏的玩法。

##### Linux

卡顿可能在桌面 Linux 上可见，但这通常与不同的视频驱动程序和合成器有关。某些合成器也可能引发此问题（例如 KWin），因此建议尝试使用不同的合成器来排除其原因。一些窗口管理器（如 KWin 和 Xfwm）允许您手动禁用合成，这可以提高性能（以撕裂为代价）。

除了将驱动程序或合成器卡顿作为问题报告给驱动程序或合成程序开发人员之外，没有解决方法。
Feral GameMode 可用于在运行特定进程时自动应用优化（例如强制 GPU 性能配置文件）。

##### macOS

一般来说，macOS 是无卡顿的，尽管最近在全屏运行时报告了一些错误（这是 macOS 错误）。如果您有一台机器表现出这种行为，请告诉我们。

##### 安卓

一般来说，安卓系统是无抖动和卡顿的，因为运行的活动获得了所有的优先级。也就是说，可能存在问题的设备（旧的 Kindle Fire 就是其中之一）。如果你在安卓系统上看到这个问题，请告诉我们。

##### iOS

iOS设备通常没有卡顿，但运行新版本操作系统的旧设备可能会出现问题。这通常是不可避免的。

#### 输入滞后

##### 项目配置

在支持禁用 V-Sync 的平台上，可以通过在项目设置中禁用 V-Sync 来减少输入滞后。然而，这会导致撕裂，尤其是在刷新率较低的显示器上。

增加每秒的物理迭代次数也可以减少物理引起的输入延迟。当使用物理插值（可以提高平滑度但增加延迟）时，这一点尤其明显。要执行此操作，请将 **“Physics” > “Common” > “Physics Ticks Per Second”** 设置为高于默认值 `60` 的值，或者在脚本中的运行时设置 `Engine.physics_ticks_per_second`。当禁用物理插值时，监视器刷新率的倍数（通常为 `60`）的值效果最好，因为它们可以避免抖动。这意味着诸如 `120`、`180` 和 `240` 之类的值是良好的起点。此外，更高的物理 FPS 使隧道和物理不稳定问题不太可能发生。

增加物理 FPS 的缺点是 CPU 使用量会增加，这可能会导致具有大量物理模拟代码的游戏出现性能瓶颈。这可以通过只在低延迟至关重要的情况下增加物理 FPS 来缓解，或者通过让玩家调整物理 FPS 以匹配他们的硬件来缓解。然而，不同的物理 FPS 将导致物理模拟中的不同结果，即使在游戏逻辑中始终使用 `delta`。这可以使某些玩家相对于其他玩家具有优势。因此，在竞争性多人游戏中，应避免允许玩家自己更改物理 FPS。

最后，可以通过在脚本中调用 `Input.set_use_accumulated_input(false)`，在每个渲染帧的基础上禁用输入缓冲。这将使脚本中的 `_input()` 和 `_unhandled_input()` 函数在每次输入时都被调用，而不是累积输入并等待渲染帧。禁用输入累积会增加 CPU 使用率，因此应谨慎操作。

##### 硬件/OS 特定

如果您的显示器支持它，请考虑在启用V-Sync的同时启用可变刷新率（G-Sync / FreeSync），然后根据此页面将项目设置中的帧速率限制为略低于显示器最大刷新率的值。例如，在 144 Hz监视器上，可以将项目的帧速率上限设置为 `141`。起初这可能违反直觉，但将FPS限制在最大刷新率范围以下可以确保操作系统永远不必等待垂直消隐完成。这导致了与使用相同帧速率上限（通常大于 1ms）禁用 V-Sync 类似的输入滞后，但没有任何撕裂。

这可以通过更改**“应用程序 > 运行 > 最大FPS项目”**设置或在脚本运行时指定 `Engine.max_fps` 来完成。

在某些平台上，您还可以在图形驱动程序选项（如 Windows 上的 NVIDIA 控制面板）中选择低延迟模式。**Ultra** 设置将以略低的平均帧速率为代价，为您提供尽可能低的延迟。强制 GPU 使用最大性能配置文件还可以进一步减少输入滞后，代价是更高的功耗（以及由此产生的热量/风扇噪音）。

最后，确保您的显示器在操作系统的显示设置中以尽可能高的刷新率运行。

此外，请确保您的鼠标配置为使用最高轮询速率（游戏鼠标通常为 1000 Hz，有时更高）。然而，高 USB 轮询率可能会导致高 CPU 使用率，因此 500 Hz 可能是低端 CPU 上更安全的选择。如果您的鼠标提供多种 DPI 设置，请考虑使用尽可能高的设置并降低游戏中的灵敏度以减少鼠标延迟。

在 Linux 上，在允许合成的窗口管理器中禁用合成（如 KWin 或 Xfwm）可以显著减少输入滞后。

#### 报告抖动、卡顿或输入滞后问题

如果您报告的卡顿或抖动问题（打开问题）不是由上述任何原因引起的，请非常清楚地指定有关设备、操作系统、驱动程序版本等的所有可能信息。这可能有助于更好地进行故障排除。
